...
ADMIN_CHAT_ID = -1003098912428  # число, как требуется для супергрупп
...
class TelegramWorkerBot:
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user_id not in worker_responses:
            # Если не было старта, попросим начать
    ...
    async def send_request(self, update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
        ...
        timestamp = user_session['timestamp'].strftime("%Y-%m-%d %H:%M:%S")
